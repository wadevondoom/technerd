...
class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.topBorder = this.add
      .rectangle(0, 0, this.ground.width, borderWidth, borderColor)
      .setOrigin(0, 0);
    this.bottomBorder = this.add
      .rectangle(
        0,
        this.ground.height - borderWidth,
        this.ground.width,
        borderWidth,
        borderColor
      )
      .setOrigin(0, 0);
    this.leftBorder = this.add
      .rectangle(0, 0, borderWidth, this.ground.height, borderColor)
      .setOrigin(0, 0);
    this.rightBorder = this.add
      .rectangle(
        this.ground.width - borderWidth,
        0,
        borderWidth,
        this.ground.height,
        borderColor
  ...
  spawnEnemy() {
    ...
    if (enemyData) {
      const enemyCar = new EnemyCar(
        ...
        enemyData.y,
        "robutt",
        this.spawnEnemyNearPlayer.bind(this)
      );
...
/**
 * This function destroys an enemy car and spawns a new one when hit by a bullet in a JavaScript game.
 * @param bullet - represents the bullet sprite that hit the enemy car.
 * @param enemyCar - The enemyCar parameter is likely an object representing an enemy car in a game.
 * The function bulletHitEnemy is likely called when a bullet collides with an enemy car, and this
 * parameter is used to reference the specific enemy car that was hit and needs to be destroyed.
 */
...
class StartScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image(
      "startScreen",
      "/static/assets/carwars/sprites/startScreen.jpg"
    );
  }
...
class GameOverScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image(
      "overScreen",
      "/static/assets/carwars/sprites/overScreen.jpg"
    );
  }
